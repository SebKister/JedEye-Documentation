...
- Downloads the new firmware, shows a live progress percentage, then reboots into it. If the radio module also needs a newer firmware, it is updated in the same step.
...
> Requires a saved WiFi network (add one under **WiFi** in the options, or with the `addwifinet` serial command) and internet access to reach the release server. If the download can't complete, the running firmware is left untouched — nothing changes until a full, valid update has been received. The USB drag-and-drop method under **Update** above remains available as a fallback.
>
